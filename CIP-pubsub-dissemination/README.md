...
3. **Resource exhaustion and denial of service.** Bounded where it runs through admission, out of scope elsewhere. The [gate](#the-verifiable-gate) limits which peers may request a link, and the [serving cap](#the-serving-cap) limits how many peer-initiated links a node accepts in an epoch, its budget sized against every declared adversarial identity dialling every eligible peer; what a flooder gains against that budget was measured rather than assumed.[^symgate] Outside that path this draft offers no defence and claims none: transport floods and amplification, verification work forced by content that fails its checks, and abuse of publication rate or payload size, whose limits [Acceptance Criteria](#acceptance-criteria) still lists as undefined. An adversary that re-registers between epochs to re-target a chosen victim is out of scope as well.
